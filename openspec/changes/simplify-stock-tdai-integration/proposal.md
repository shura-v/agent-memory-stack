...
- Replace persistent MCP credential/session pools with request-scoped stateless Streamable HTTP: each authenticated HTTP POST owns an isolated unchanged stdio child, closed when its response ends or aborts. Retain user/resource authorization and the existing `/mcp` URL; remove session maps, leases and idle expiry. Bound the gateway to 64 active children and evict the oldest active request when admitting a new one at capacity.
...
Touches build/source acquisition and Docker recipes, native configuration lifecycle and transport, Compose/readiness, MCP request transport/bridge, tests and operator docs. The six applications remain; helpers reduce to `config`, `bootstrap` and `access`, with no additional publicly published backend.

This change supersedes conflicting decisions in the unfinished `preserve-native-tdai-configs` change and the custom tool contract in `add-server-mcp-gateway`. It retains two visible configuration sets, user-owned values, Configure-only models and the fixed full stack. There have been no releases: no migrations or compatibility shims are required. At proposal time, partial working-tree edits were inputs to review, not evidence of completion. Implementation evidence is recorded in validation.md. The 2026-09-19 documentation refresh synchronized current main specs and active deltas; no change was archived.
