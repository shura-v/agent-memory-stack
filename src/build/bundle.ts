...
import { imageServices, validateDeploymentImages } from './images.js';
import type { ContainerRuntime, ImageManifest, ImagePlatform } from './images.js';
import { acquireSourceArchive, cacheSourceArchive, loadSourceLock, validateTdaiSource, verifyArchive } from './sources.js';
...
const tdaiServices = ['core', 'knowledge', 'panel', 'memory-proxy', 'mcp'];
function verifySourceRevision(service: string, labels: Record<string, string> | undefined, source: SourceLock): void {
  if (tdaiServices.includes(service) && labels?.['org.opencontainers.image.revision'] !== source.revision) {
...
export async function exportImages({ projectDir, outputDir, runtime }: ExportOptions): Promise<void> {
  ...
  const manifest = validateDeploymentImages(saved);
  const source = validateTdaiSource((await loadSourceLock(projectDir)).sources.tencent);
  verifyLocalImages(runtime, manifest, source);
  const sourceArchive = await acquireSourceArchive(projectDir, 'tencent', source);
  const sourceBytes = await readFile(sourceArchive);
  verifyArchive(sourceBytes, source.sha256, 'TDAI bundle source');
  await extractNativeTemplates(sourceArchive, source);
  await mkdir(dirname(resolve(outputDir)), { recursive: true });
  await mkdir(outputDir, { recursive: false });
  const archive = resolve(outputDir, 'images.tar');
  const ids = [...new Set(imageServices.map(service => manifest.images[service]!.id))];
  ...
  await writeFile(resolve(outputDir, 'tdai-source.tar.gz'), sourceBytes);
  const metadata: BundleMetadata = {
    schemaVersion: 1, archiveSha256: await fileHash(archive), manifestSha256: await fileHash(resolve(outputDir, 'images.json')),
    tdaiSourceSha256: await fileHash(resolve(outputDir, 'tdai-source.json')),
  };
...
export async function loadImages({ bundleDir, projectDir, runtime, platform }: LoadOptions): Promise<ImageManifest> {
  ...
  const manifest = validateDeploymentImages(JSON.parse(await readFile(resolve(bundleDir, 'images.json'), 'utf8')));
  if (metadata.tdaiSourceSha256 === undefined) throw new Error('TDAI image bundle requires source metadata');
  ...
  const source = validateTdaiSource(JSON.parse(sourceBytes.toString('utf8')));
  verifyArchiveImages(archive, manifest, source);
  const sourceArchive = resolve(bundleDir, 'tdai-source.tar.gz');
  const archiveBytes = await readFile(sourceArchive);
  verifyArchive(archiveBytes, source.sha256, 'TDAI bundle source');
  await extractNativeTemplates(sourceArchive, source);
  ...
  validateDeploymentImages(manifest, actualPlatform);
  await run(runtime, ['load', '--input', archive]);
  verifyLocalImages(runtime, manifest, source);
  await mkdir(resolve(projectDir, '.ams'), { recursive: true });
  await cacheSourceArchive(projectDir, 'tencent', source, archiveBytes);
  await atomicWrite(resolve(projectDir, '.ams/pending-tdai-source.json'), `${JSON.stringify(source, null, 2)}\n`);
  await atomicWrite(resolve(projectDir, '.ams/pending-images.json'), `${JSON.stringify(manifest, null, 2)}\n`);
