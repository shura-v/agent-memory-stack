...
import { buildImages, imageServices } from './images.js';
import type { ContainerRuntime, ImagePlatform, ImageService } from './images.js';
import { fetchSources } from './sources.js';
import { exportImages, loadImages } from './bundle.js';

try {
  const { values, positionals } = parseArgs({ options: { runtime: { type: 'string', default: 'docker' }, platform: { type: 'string' }, 'project-dir': { type: 'string', default: process.cwd() }, fetch: { type: 'boolean', default: false }, export: { type: 'string' }, load: { type: 'string' } }, allowPositionals: true });
  ...
  if (values.fetch) await fetchSources(values['project-dir']);
    ...
    if (positionals.some(service => !imageServices.includes(service as ImageService))) throw new Error(`Services: ${imageServices.join(', ')}`);
    await buildImages({ projectDir: values['project-dir'], runtime: values.runtime as ContainerRuntime, platform: values.platform as ImagePlatform | undefined, services: positionals as ImageService[] });
  }
} catch (error) {
