...
export interface ProbeConfig {
  checks: Array<{ name: string; kind: 'core' | 'model' | 'knowledge' | 'panel'; url: string; key: string; serviceId?: string }>;
  ...
  for (const entry of config.checks) await check(entry.name, async () => {
    ...
    if (entry.kind !== 'model') {
      ...
      if (entry.kind === 'core' && (identity.knowledgeId || identity.panelId)
        || entry.kind === 'knowledge' && (!identity.knowledgeId || identity.panelId)
